...
# ViewModel: add a dedicated drawing-data refresh command.
...
'@ -NewText @'
        public void RefreshDrawingDataFromDrawing()
        {
            ...
            {
                ConnectionStatus = $"Ошибка получения данных с чертежа: {ex.Message}";
            }
        }
'@

# Main window: add compact refresh button to the active profile toolbar.
...
'@ -NewText @'
					<Button Grid.Column="2" Command="{Binding RefreshDrawingDataCommand}" Width="32" Height="28" Padding="4" Margin="0,0,8,0" ToolTip.Tip="Получить данные с чертежа">
...
# Commands: keep one shared ViewModel instance and refresh drawing data immediately on SPEC_SHOW.
...
Write-Host "Applied drawing data refresh pipeline changes." -ForegroundColor Green
